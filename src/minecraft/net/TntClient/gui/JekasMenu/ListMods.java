...
public class ListMods extends GuiScreen {
    ...
    private Module[] visibleList = Config.modules;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        final int size = visibleList.length;
        ...
        for (int i = 0; i < size; i++) {
            final boolean isOptions = visibleList[i].getOptions().size() > 0;
            ...
            if (visibleList[i].isBlocking)
                glColor4f(0.5f, 0.5f, 0.5f, .5f);
            else if (visibleList[i].isActive())
                glColor4f(0, 1, 0, .5f);
            else
            ...
            mc.fontRendererObj.drawString(mc.fontRendererObj.trimStringToWidth((visibleList[i].keyBind !=
                    Integer.MAX_VALUE ? ("[" + Keyboard.getKeyName(visibleList[i].keyBind) + "]") : "") +
                    visibleList[i].getName(), whid - (isOptions ? 12 : 0) - 5), PosX + 5, PosY +
        ...
        if (selected != -1) {
            final Module md = visibleList[selected];
            if (!md.isActive()) {
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        ...
        for (Module m : Config.modules)
            if (m.getName().toLowerCase().contains(searchText))
                modules.add(m);
        visibleList = modules.toArray(new Module[0]);
    }
    ...
    protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
        if (isSellectScroll)
            scroll = Math.min(0,
                    Math.max(-((visibleList.length - 1) / 3 * blockHeight + (17 + blockHeight - height)),
                            -(((visibleList.length + 2) * blockHeight * (2 * mouseY + height - new ScaledResolution(
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (mouseButton == 0) {
            ...
            if (mouseY > ScY + 13 && mouseY < ScY + height && mouseX > ScX && mouseX < ScX + width) {
                ...
                final int whid = visibleList.length > 12 ? (width - 5) / 3 : width / 3;
                for (int i = 0; i < visibleList.length; i++) {
                    if (visibleList[i].isBlocking) continue;
                    final boolean isOptions = visibleList[i].getOptions().size() > 0;
                    final int PosX = (i % 3) * whid + ScX + 2;
            ...
            if (mouseY > ScY + 13 && mouseY < ScY + height && mouseX > ScX && mouseX < ScX + width) {
                ...
                for (int i = 0; i < visibleList.length; i++) {
                    ...
                    if (mouseY > PosY && mouseY < PosY + blockHeight - 2 && mouseX > PosX && mouseX < PosX + whid) {
                        mc.displayGuiScreen(new KeyBind(visibleList[i]));
                    }
                }
            }
        }
        ...
        if (visibleList.length > 12) {
            ...
            if (i != 0) {
                ...
                if (scroll > 0)
                    ...
                    final int itemLen = -((visibleList.length - 1) / 3 * blockHeight + (17 + blockHeight - height));
                    if (scroll < itemLen)
                        scroll = itemLen;
                }
            }
