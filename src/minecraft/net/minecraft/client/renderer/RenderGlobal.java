...
public class RenderGlobal implements IWorldAccess, IResourceManagerReloadListener {
    ...
    public void setupTerrain(Entity viewEntity, double partialTicks, ICamera camera, int frameCount, boolean playerSpectator) {
        ...
        if (!flag && this.displayListEntitiesDirty && !Shaders.isShadowPass) {
            ...
            if (renderchunk != null) {
                ...
                if (set1.isEmpty() && !playerSpectator) {
                ...
                for (int j = -this.renderDistanceChunks; j <= this.renderDistanceChunks; ++j) {
                    for (int k = -this.renderDistanceChunks; k <= this.renderDistanceChunks; ++k) {
                        ...
                        if (renderchunk2 != null && camera.isBoundingBoxInFrustum(renderchunk2.boundingBox)) {
                            ...
                            deque.add(new RenderGlobal.ContainerLocalRenderInformation(renderchunk2, null, 0));
                        }
                    }
                }
            }
                ...
                for (int i1 = 0; i1 < l; ++i1) {
                    ...
                    if ((!flag1 || !renderglobal$containerlocalrenderinformation1.setFacing.contains(enumfacing1.getOpposite())) && (!flag1 || enumfacing2 == null || renderchunk4.getCompiledChunk().isVisible(enumfacing2.getOpposite(), enumfacing1))) {
                        ...
                        if (renderchunk3 != null && renderchunk3.setFrameIndex(frameCount) && camera.isBoundingBoxInFrustum(renderchunk3.boundingBox)) {
                            RenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation = new RenderGlobal.ContainerLocalRenderInformation(renderchunk3, enumfacing1, renderglobal$containerlocalrenderinformation1.counter + 1);
    ...
    private void renderBlockLayer(EnumWorldBlockLayer blockLayerIn) {
        ...
        if (OpenGlHelper.useVbo()) {
            for (VertexFormatElement vertexformatelement : DefaultVertexFormats.BLOCK.getElements()) {
                switch (RenderGlobal.RenderGlobal$2.field_178037_a[vertexformatelement.getUsage().ordinal()]) {
                    case 1:
    ...
    public void renderSky(float partialTicks, int pass) {
        ...
        if (this.mc.theWorld.provider.getDimensionId() == 1) {
            ...
            GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
            RenderHelper.disableStandardItemLighting();
            float[] afloat = this.theWorld.provider.calcSunriseSunsetColors(this.theWorld.getCelestialAngle(partialTicks));
            ...
            CustomSky.renderSky(this.theWorld, this.theWorld.getCelestialAngle(partialTicks), f15);
