...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.theWorld != null) {
            ...
            Vec3 vec33 = null;
            float f = 1.0F;
            List list = this.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox()
    ...
    public void func_181560_a(float p_181560_1_, long p_181560_2_) {
        ...
        if (this.mc.inGameHasFocus && flag) {
            ...
            this.mc.thePlayer.setAngles(f2, f3 * (mc.gameSettings.invertMouse ? -1 : 1));
        }
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ...
                    crashreportcategory.addCrashSectionCallable("Screen size", (Callable) () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d", scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight(), EntityRenderer.this.mc.displayWidth, EntityRenderer.this.mc.displayHeight, scaledresolution.getScaleFactor()));
                ...
                for (int j1 = i - b0; j1 <= i + b0; ++j1) {
                    ...
                    if (biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow()) {
                        ...
                        if (i2 != j2) {
                            ...
                            if (worldclient.getWorldChunkManager().getTemperatureAtHeight(f1) >= 0.15F) {
                                if (b1 != 0) {
                                    if (b1 >= 0) {
                                        tessellator.draw();
                                    }
    ...
    private void updateFogColor(float partialTicks) {
        ...
        vec3 = CustomColors.getWorldSkyColor(vec3, worldclient, this.mc.getRenderViewEntity());
        float f1 = (float) vec3.xCoord;
        float f2 = (float) vec3.yCoord;
        float f3 = (float) vec3.zCoord;
        Vec3 vec31 = worldclient.getFogColor(partialTicks);
        vec31 = CustomColors.getWorldFogColor(vec31, worldclient, this.mc.getRenderViewEntity());
        ...
        if (this.mc.gameSettings.renderDistanceChunks >= 4) {
            ...
            if (f4 > 0.0F) {
                float[] afloat = worldclient.provider.calcSunriseSunsetColors(worldclient.getCelestialAngle(partialTicks));
    ...
    private void setupFog(int p_78468_1_, float partialTicks) {
        ...
        if (f1 >= 0.0F) {
            ...
            if (this.mc.theWorld.provider.doesXZShowFog()) {
