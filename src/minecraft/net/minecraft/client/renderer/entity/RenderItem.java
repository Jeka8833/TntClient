    ...
    private void renderQuads(WorldRenderer renderer, List<BakedQuad> quads, int color, ItemStack stack) {
        ...
        for (int j = quads.size(); i < j; ++i) {
            ...
            if (renderer.isMultiTexture()) {
                renderer.addVertexData(bakedquad.getVertexDataSingle());
                renderer.putSprite(bakedquad.getSprite());
            } else {
            ...
            if (Reflector.IColoredBakedQuad.exists() && Reflector.IColoredBakedQuad.isInstance(bakedquad)) {
                forgeHooksClient_putQuadColor(renderer, bakedquad, k);
            } else {
                int r = k >> 16 & 255;
                int g = k >> 8 & 255;
                int b = k & 255;
                ...
                for (int ia = 1; ia < 5; ++ia)
                    renderer.putColorRGBA(renderer.getColorIndex(ia), r, g, b, a);
            }
    ...
    public void renderItemOverlayIntoGUI(FontRenderer fr, ItemStack stack, int xPosition, int yPosition, String text) {
        if (stack != null) {
            ...
            if (flag) {
                ...
                Tessellator tessellator = Tessellator.getInstance();
                WorldRenderer worldrenderer = tessellator.getWorldRenderer();
                this.func_181565_a(worldrenderer, xPosition + 2, yPosition + 13, 13, 2, 0, 0);
                this.func_181565_a(worldrenderer, xPosition + 2, yPosition + 13, 12, 1, (255 - j) / 4, 64);
                this.func_181565_a(worldrenderer, xPosition + 2, yPosition + 13, i, 1, 255 - j, j);
