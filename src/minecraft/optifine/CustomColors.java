...
public class CustomColors {
    ...
    private static final IColorizer COLORIZER_WATER = new IColorizer() {
        public int getColor(IBlockAccess p_getColor_1_, BlockPos p_getColor_2_) {
            BiomeGenBase biomegenbase = CustomColors.getColorBiome(p_getColor_1_, p_getColor_2_);
            return CustomColors.waterColors != null ? CustomColors.waterColors.getColor(biomegenbase, p_getColor_2_) : (Reflector.ForgeBiome_getWaterColorMultiplier.exists() ? Reflector.callInt(biomegenbase, Reflector.ForgeBiome_getWaterColorMultiplier) : biomegenbase.waterColorMultiplier);
        }
    ...
    public static void update() {
        ...
        if (Config.isCustomColors()) {
            ...
            getCustomColors(s + "durability.png", -1, -1);
    ...
    private static void readColorProperties() {
        try {
            ...
            colorsBlockColormaps = readCustomColormaps(properties);
            spawnEggPrimaryColors = readSpawnEggColors(properties, "egg.shell.", "Spawn egg shell");
            spawnEggSecondaryColors = readSpawnEggColors(properties, "egg.spots.", "Spawn egg spot");
            wolfCollarColors = readDyeColors(properties, "collar.", "Wolf collar");
            sheepColors = readDyeColors(properties, "sheep.", "Sheep");
            textColors = readTextColors(properties);
            int[] aint = readMapColors(properties);
    ...
    private static CustomColormap[] readCustomColormaps(Properties p_readCustomColormaps_0_) {
        ...
        for (int j = 0; j < astring.length; ++j) {
            ...
            String s5 = TextureUtils.getBasePath("mcpatcher/color.properties");
    ...
    private static CustomColormap[][] readBlockColormaps(String[] p_readBlockColormaps_0_, CustomColormap[] p_readBlockColormaps_1_) {
    ...
    public static int getColorMultiplier(BakedQuad p_getColorMultiplier_0_, Block p_getColorMultiplier_1_, IBlockAccess p_getColorMultiplier_2_, BlockPos p_getColorMultiplier_3_, RenderEnv p_getColorMultiplier_4_) {
        ...
        if (!p_getColorMultiplier_0_.hasTintIndex()) {
            ...
            return getStemColorMultiplier(p_getColorMultiplier_1_, p_getColorMultiplier_4_);
    ...
    private static int getRedstoneColor(IBlockState p_getRedstoneColor_0_) {
        if (redstoneColors == null) {
            ...
            int i = getRedstoneLevel(p_getRedstoneColor_0_);
    ...
    private static int getRedstoneLevel(IBlockState p_getRedstoneLevel_0_) {
    ...
    public static void updateWaterFX(EntityFX p_updateWaterFX_0_, IBlockAccess p_updateWaterFX_1_, double p_updateWaterFX_2_, double p_updateWaterFX_4_, double p_updateWaterFX_6_) {
        if (waterColors != null || blockColormaps != null) {
            ...
            RenderEnv renderenv = RenderEnv.getInstance(BLOCK_STATE_WATER);
    ...
    public static Vec3 getUnderwaterColor(IBlockAccess p_getUnderwaterColor_0_, double p_getUnderwaterColor_1_, double p_getUnderwaterColor_3_, double p_getUnderwaterColor_5_) {
        if (underwaterColors == null) {
            ...
            float f1 = (float) k / 255.0F;
            float f2 = (float) l / 255.0F;
            return underwaterColorFader.getColor(f, f1, f2);
        }
    }

    private static int getStemColorMultiplier(Block p_getStemColorMultiplier_0_, RenderEnv p_getStemColorMultiplier_3_) {
    ...
    private static void getLightMapColumn(float[][] p_getLightMapColumn_0_, float p_getLightMapColumn_1_, int p_getLightMapColumn_2_, int p_getLightMapColumn_3_, float[][] p_getLightMapColumn_4_) {
        ...
        int j = (int) Math.ceil(p_getLightMapColumn_1_);
    ...
    public static Vec3 getWorldFogColor(Vec3 p_getWorldFogColor_0_, WorldClient p_getWorldFogColor_1_, Entity p_getWorldFogColor_2_) {
    ...
    public static Vec3 getWorldSkyColor(Vec3 p_getWorldSkyColor_0_, World p_getWorldSkyColor_1_, Entity p_getWorldSkyColor_2_) {
    ...
    private static int getSpawnEggColor(ItemStack p_getSpawnEggColor_1_, int p_getSpawnEggColor_2_, int p_getSpawnEggColor_3_) {
    ...
    public static int getColorFromItemStack(ItemStack p_getColorFromItemStack_0_, int p_getColorFromItemStack_1_, int p_getColorFromItemStack_2_) {
        if (p_getColorFromItemStack_0_ == null) {
            ...
            return item == null ? p_getColorFromItemStack_2_ : (item instanceof ItemMonsterPlacer ? getSpawnEggColor(p_getColorFromItemStack_0_, p_getColorFromItemStack_1_, p_getColorFromItemStack_2_) : p_getColorFromItemStack_2_);
        }
    }
    ...
    private static int[] readTextColors(Properties p_readTextColors_0_) {
        int[] aint = new int[32];
        Arrays.fill(aint, -1);
        int i = 0;
        ...
        for (Object s : p_readTextColors_0_.keySet()) {
            ...
            if (((String) s).startsWith("text.code.")) {
                String s2 = StrUtils.removePrefix((String) s, "text.code.");
    ...
    private static int[] readMapColors(Properties p_readMapColors_0_) {
        ...
        for (Object s : p_readMapColors_0_.keySet()) {
            ...
            if (((String) s).startsWith("map.")) {
                String s2 = StrUtils.removePrefix((String) s, "map.");
    ...
    private static int[] readPotionColors(Properties p_readPotionColors_0_) {
