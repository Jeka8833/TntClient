...
public class CustomColormap implements CustomColors.IColorizer {
   ...
   public boolean isValid(String path) {
      if (this.format != 0 && this.format != 1) {
         ...
         if (this.color < 0) {
            ...
            if (this.format == 1) {
               this.color = this.getColorGrid(BiomeGenBase.field_76772_c, new BlockPos(0, 64, 0));
            }
         }
      }
   ...
   private MatchBlock[] detectMatchBlocks() {
      Block block = Block.func_149684_b(this.name);
      if (block != null) {
         return new MatchBlock[]{new MatchBlock(Block.func_149682_b(block))};
      } else {
   ...
   public int getColorSmooth(IBlockAccess blockAccess, double x, double y, double z, int radius) {
      if (this.format == 2) {
         ...
         int x0 = MathHelper.func_76128_c(x);
         int y0 = MathHelper.func_76128_c(y);
         int z0 = MathHelper.func_76128_c(z);
         int sumRed = 0;
         int sumGreen = 0;
         int sumBlue = 0;
         int count = 0;
         ...
         for(ix = x0 - radius; ix <= x0 + radius; ++ix) {
            for(iz = z0 - radius; iz <= z0 + radius; ++iz) {
               ...
               col = this.getColor((IBlockAccess)blockAccess, blockPosM);
               sumRed += col >> 16 & 255;
               sumGreen += col >> 8 & 255;
               sumBlue += col & 255;
               ++count;
   ...
   private int getColorVanilla(BiomeGenBase biome, BlockPos blockPos) {
      double temperature = (double)MathHelper.func_76131_a(biome.func_180626_a(blockPos), 0.0F, 1.0F);
      double rainfall = (double)MathHelper.func_76131_a(biome.func_76727_i(), 0.0F, 1.0F);
   ...
   private int getColorGrid(BiomeGenBase biome, BlockPos blockPos) {
      int cx = biome.field_76756_M;
      int cy = blockPos.func_177956_o() - this.yOffset;
      if (this.yVariance > 0) {
         int seed = blockPos.func_177958_n() << 16 + blockPos.func_177952_p();
