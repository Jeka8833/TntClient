...
public class NbtTagValue {
   ...
   public boolean matchesCompound(NBTTagCompound nbt) {
      if (nbt == null) {
         ...
         if (this.name.equals("*")) {
            ...
            NBTBase tagBase = getChildTag((NBTBase)tagBase, this.name);
            if (tagBase == null) {
               return false;
   ...
   private boolean matchesAnyChild(NBTBase tagBase) {
      if (tagBase instanceof NBTTagCompound) {
         ...
         while(it.hasNext()) {
            ...
            NBTBase nbtBase = tagCompound.func_74781_a(key);
      ...
      if (tagBase instanceof NBTTagList) {
         ...
         for(int i = 0; i < count; ++i) {
            NBTBase nbtBase = tagList.func_179238_g(i);
            if (this.matchesBase(nbtBase)) {
               return true;
            }
         }
   ...
   private static NBTBase getChildTag(NBTBase tagBase, String tag) {
      if (tagBase instanceof NBTTagCompound) {
         NBTTagCompound tagCompound = (NBTTagCompound)tagBase;
         return tagCompound.func_74781_a(tag);
      } else if (tagBase instanceof NBTTagList) {
         NBTTagList tagList = (NBTTagList)tagBase;
         if (tag.equals("count")) {
            return new NBTTagInt(tagList.func_74745_c());
         } else {
            int index = Config.parseInt(tag, -1);
            return index >= 0 && index < tagList.func_74745_c() ? tagList.func_179238_g(index) : null;
         }
      } else {
         return null;
      }
