   ...
   public float eval(int index1, int index2) {
      if (this.indexNames1 == null || index1 >= 0 && index1 <= this.indexNames1.length) {
         if (this.indexNames2 != null && (index2 < 0 || index2 > this.indexNames2.length)) {
            ...
            switch(this) {
            case BIOME:
               pos = Shaders.getCameraPosition();
               biome = Shaders.getCurrentWorld().func_180494_b(pos);
               return (float)biome.field_76756_M;
            case TEMPERATURE:
               pos = Shaders.getCameraPosition();
               biome = Shaders.getCurrentWorld().func_180494_b(pos);
               return biome != null ? biome.func_180626_a(pos) : 0.0F;
            case RAINFALL:
               pos = Shaders.getCameraPosition();
               biome = Shaders.getCurrentWorld().func_180494_b(pos);
               return biome != null ? biome.func_76727_i() : 0.0F;
            default:
