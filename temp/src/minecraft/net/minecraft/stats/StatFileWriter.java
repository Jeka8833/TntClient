package net.minecraft.stats;

import com.google.common.collect.Maps;
import java.util.Map;
import net.minecraft.entity.player.EntityPlayer;
import net.minecraft.util.IJsonSerializable;
import net.minecraft.util.TupleIntJsonSerializable;

public class StatFileWriter {
   protected final Map<StatBase, TupleIntJsonSerializable> field_150875_a = Maps.newConcurrentMap();

   public boolean func_77443_a(Achievement p_77443_1_) {
      return this.func_77444_a(p_77443_1_) > 0;
   }

   public boolean func_77442_b(Achievement p_77442_1_) {
      return p_77442_1_.field_75992_c == null || this.func_77443_a(p_77442_1_.field_75992_c);
   }

   public int func_150874_c(Achievement p_150874_1_) {
      if (this.func_77443_a(p_150874_1_)) {
         return 0;
      } else {
         int lvt_2_1_ = 0;

         for(Achievement lvt_3_1_ = p_150874_1_.field_75992_c; lvt_3_1_ != null && !this.func_77443_a(lvt_3_1_); ++lvt_2_1_) {
            lvt_3_1_ = lvt_3_1_.field_75992_c;
         }

         return lvt_2_1_;
      }
   }

   public void func_150871_b(EntityPlayer p_150871_1_, StatBase p_150871_2_, int p_150871_3_) {
      if (!p_150871_2_.func_75967_d() || this.func_77442_b((Achievement)p_150871_2_)) {
         this.func_150873_a(p_150871_1_, p_150871_2_, this.func_77444_a(p_150871_2_) + p_150871_3_);
      }
   }

   public void func_150873_a(EntityPlayer p_150873_1_, StatBase p_150873_2_, int p_150873_3_) {
      TupleIntJsonSerializable lvt_4_1_ = (TupleIntJsonSerializable)this.field_150875_a.get(p_150873_2_);
      if (lvt_4_1_ == null) {
         lvt_4_1_ = new TupleIntJsonSerializable();
         this.field_150875_a.put(p_150873_2_, lvt_4_1_);
      }

      lvt_4_1_.func_151188_a(p_150873_3_);
   }

   public int func_77444_a(StatBase p_77444_1_) {
      TupleIntJsonSerializable lvt_2_1_ = (TupleIntJsonSerializable)this.field_150875_a.get(p_77444_1_);
      return lvt_2_1_ == null ? 0 : lvt_2_1_.func_151189_a();
   }

   public <T extends IJsonSerializable> T func_150870_b(StatBase p_150870_1_) {
      TupleIntJsonSerializable lvt_2_1_ = (TupleIntJsonSerializable)this.field_150875_a.get(p_150870_1_);
      return lvt_2_1_ != null ? lvt_2_1_.func_151187_b() : null;
   }

   public <T extends IJsonSerializable> T func_150872_a(StatBase p_150872_1_, T p_150872_2_) {
      TupleIntJsonSerializable lvt_3_1_ = (TupleIntJsonSerializable)this.field_150875_a.get(p_150872_1_);
      if (lvt_3_1_ == null) {
         lvt_3_1_ = new TupleIntJsonSerializable();
         this.field_150875_a.put(p_150872_1_, lvt_3_1_);
      }

      lvt_3_1_.func_151190_a(p_150872_2_);
      return p_150872_2_;
   }
}
