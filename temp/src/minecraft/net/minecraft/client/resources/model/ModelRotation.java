...
public enum ModelRotation implements IModelState, ITransformation {
   ...
   public EnumFacing func_177523_a(EnumFacing p_177523_1_) {
      ...
      int j;
      for(j = 0; j < this.field_177543_t; ++j) {
         enumfacing = enumfacing.func_176732_a(EnumFacing.Axis.X);
      }

      if (enumfacing.func_176740_k() != EnumFacing.Axis.Y) {
         for(j = 0; j < this.field_177542_u; ++j) {
   ...
   public javax.vecmath.Matrix4f getMatrix() {
      return Reflector.ForgeHooksClient_getMatrix.exists() ? (javax.vecmath.Matrix4f)Reflector.call(Reflector.ForgeHooksClient_getMatrix, this) : new javax.vecmath.Matrix4f(this.func_177525_a());
   }
   ...
   static {
      ModelRotation[] arr$ = values();
      int len$ = arr$.length;

      for(int i$ = 0; i$ < len$; ++i$) {
         ModelRotation modelrotation = arr$[i$];
