...
public class GlStateManager {
   ...
   public static void func_179150_h(int p_179150_0_) {
      if (p_179150_0_ != 0) {
         ...
         GlStateManager.TextureState[] arr$ = field_179174_p;
         int len$ = arr$.length;

         for(int i$ = 0; i$ < len$; ++i$) {
            GlStateManager.TextureState glstatemanager$texturestate = arr$[i$];
   ...
   public static void deleteTextures(IntBuffer p_deleteTextures_0_) {
      ...
      while(p_deleteTextures_0_.position() < p_deleteTextures_0_.limit()) {
         int texId = p_deleteTextures_0_.get();
         func_179150_h(texId);
      }
   ...
   static {
      int j;
      for(j = 0; j < 8; ++j) {
         field_179159_c[j] = new GlStateManager.BooleanState(16384 + j);
      }

      for(j = 0; j < field_179174_p.length; ++j) {
