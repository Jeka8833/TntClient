package net.minecraft.entity;

import java.util.Collection;
import java.util.Iterator;
import java.util.UUID;
import net.minecraft.entity.ai.attributes.AttributeModifier;
import net.minecraft.entity.ai.attributes.BaseAttributeMap;
import net.minecraft.entity.ai.attributes.IAttribute;
import net.minecraft.entity.ai.attributes.IAttributeInstance;
import net.minecraft.entity.ai.attributes.RangedAttribute;
import net.minecraft.nbt.NBTTagCompound;
import net.minecraft.nbt.NBTTagList;
import org.apache.logging.log4j.LogManager;
import org.apache.logging.log4j.Logger;

public class SharedMonsterAttributes {
   private static final Logger field_151476_f = LogManager.getLogger();
   public static final IAttribute field_111267_a = (new RangedAttribute((IAttribute)null, "generic.maxHealth", 20.0D, 0.0D, 1024.0D)).func_111117_a("Max Health").func_111112_a(true);
   public static final IAttribute field_111265_b = (new RangedAttribute((IAttribute)null, "generic.followRange", 32.0D, 0.0D, 2048.0D)).func_111117_a("Follow Range");
   public static final IAttribute field_111266_c = (new RangedAttribute((IAttribute)null, "generic.knockbackResistance", 0.0D, 0.0D, 1.0D)).func_111117_a("Knockback Resistance");
   public static final IAttribute field_111263_d = (new RangedAttribute((IAttribute)null, "generic.movementSpeed", 0.699999988079071D, 0.0D, 1024.0D)).func_111117_a("Movement Speed").func_111112_a(true);
   public static final IAttribute field_111264_e = new RangedAttribute((IAttribute)null, "generic.attackDamage", 2.0D, 0.0D, 2048.0D);

   public static NBTTagList func_111257_a(BaseAttributeMap p_111257_0_) {
      NBTTagList lvt_1_1_ = new NBTTagList();
      Iterator lvt_2_1_ = p_111257_0_.func_111146_a().iterator();

      while(lvt_2_1_.hasNext()) {
         IAttributeInstance lvt_3_1_ = (IAttributeInstance)lvt_2_1_.next();
         lvt_1_1_.func_74742_a(func_111261_a(lvt_3_1_));
      }

      return lvt_1_1_;
   }

   private static NBTTagCompound func_111261_a(IAttributeInstance p_111261_0_) {
      NBTTagCompound lvt_1_1_ = new NBTTagCompound();
      IAttribute lvt_2_1_ = p_111261_0_.func_111123_a();
      lvt_1_1_.func_74778_a("Name", lvt_2_1_.func_111108_a());
      lvt_1_1_.func_74780_a("Base", p_111261_0_.func_111125_b());
      Collection<AttributeModifier> lvt_3_1_ = p_111261_0_.func_111122_c();
      if (lvt_3_1_ != null && !lvt_3_1_.isEmpty()) {
         NBTTagList lvt_4_1_ = new NBTTagList();
         Iterator lvt_5_1_ = lvt_3_1_.iterator();

         while(lvt_5_1_.hasNext()) {
            AttributeModifier lvt_6_1_ = (AttributeModifier)lvt_5_1_.next();
            if (lvt_6_1_.func_111165_e()) {
               lvt_4_1_.func_74742_a(func_111262_a(lvt_6_1_));
            }
         }

         lvt_1_1_.func_74782_a("Modifiers", lvt_4_1_);
      }

      return lvt_1_1_;
   }

   private static NBTTagCompound func_111262_a(AttributeModifier p_111262_0_) {
      NBTTagCompound lvt_1_1_ = new NBTTagCompound();
      lvt_1_1_.func_74778_a("Name", p_111262_0_.func_111166_b());
      lvt_1_1_.func_74780_a("Amount", p_111262_0_.func_111164_d());
      lvt_1_1_.func_74768_a("Operation", p_111262_0_.func_111169_c());
      lvt_1_1_.func_74772_a("UUIDMost", p_111262_0_.func_111167_a().getMostSignificantBits());
      lvt_1_1_.func_74772_a("UUIDLeast", p_111262_0_.func_111167_a().getLeastSignificantBits());
      return lvt_1_1_;
   }

   public static void func_151475_a(BaseAttributeMap p_151475_0_, NBTTagList p_151475_1_) {
      for(int lvt_2_1_ = 0; lvt_2_1_ < p_151475_1_.func_74745_c(); ++lvt_2_1_) {
         NBTTagCompound lvt_3_1_ = p_151475_1_.func_150305_b(lvt_2_1_);
         IAttributeInstance lvt_4_1_ = p_151475_0_.func_111152_a(lvt_3_1_.func_74779_i("Name"));
         if (lvt_4_1_ != null) {
            func_111258_a(lvt_4_1_, lvt_3_1_);
         } else {
            field_151476_f.warn("Ignoring unknown attribute '" + lvt_3_1_.func_74779_i("Name") + "'");
         }
      }

   }

   private static void func_111258_a(IAttributeInstance p_111258_0_, NBTTagCompound p_111258_1_) {
      p_111258_0_.func_111128_a(p_111258_1_.func_74769_h("Base"));
      if (p_111258_1_.func_150297_b("Modifiers", 9)) {
         NBTTagList lvt_2_1_ = p_111258_1_.func_150295_c("Modifiers", 10);

         for(int lvt_3_1_ = 0; lvt_3_1_ < lvt_2_1_.func_74745_c(); ++lvt_3_1_) {
            AttributeModifier lvt_4_1_ = func_111259_a(lvt_2_1_.func_150305_b(lvt_3_1_));
            if (lvt_4_1_ != null) {
               AttributeModifier lvt_5_1_ = p_111258_0_.func_111127_a(lvt_4_1_.func_111167_a());
               if (lvt_5_1_ != null) {
                  p_111258_0_.func_111124_b(lvt_5_1_);
               }

               p_111258_0_.func_111121_a(lvt_4_1_);
            }
         }
      }

   }

   public static AttributeModifier func_111259_a(NBTTagCompound p_111259_0_) {
      UUID lvt_1_1_ = new UUID(p_111259_0_.func_74763_f("UUIDMost"), p_111259_0_.func_74763_f("UUIDLeast"));

      try {
         return new AttributeModifier(lvt_1_1_, p_111259_0_.func_74779_i("Name"), p_111259_0_.func_74769_h("Amount"), p_111259_0_.func_74762_e("Operation"));
      } catch (Exception var3) {
         field_151476_f.warn("Unable to create attribute: " + var3.getMessage());
         return null;
      }
   }
}
